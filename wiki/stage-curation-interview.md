...
The dry-run (Klee, three works) validated a reliable per-work arc: **observe → hypothesize the rule → (redirect narrative → technique) → commit + design the drill → confirm**. The convergence move is *"if you copy this to steal that one rule, what are you practicing, and what's the test that proves you learned it vs. just a nice copy?"* — it forced a practiceable lesson out of every image and maps directly onto [[20-dimensions-of-study-cheat-sheet]]'s closing layer (*one thing to copy · one to analyze · one sentence lesson: "this artist teaches me how to ___"*). This operationalizes the master-study pedagogy in [[05.1-master-study-pedagogy]]: a copy is only instructive when it isolates a transferable decision.
