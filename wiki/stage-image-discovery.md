...
Discovery now writes into persisted package state (Thrust 1, built): the merged board is stored as `candidates[]` (each entry tagged `origin: discovered|user` — the `user` value is the seam for the not-yet-built Thrust 2 custom-image source) and every run is appended to `runs[]`. Re-running discovery merges rather than replaces, so a later run after a WDQS outage extends the same board.
