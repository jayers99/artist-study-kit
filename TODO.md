...
- [ ] **Duplicate handling on re-query/ingest.** As the gallery grows across discovery runs + custom-image imports, define what happens when a newly-found image is (or may be) a duplicate of one already on the board — so a starred work isn't re-added unstarred. Deferred out of Thrust 3; needs its own spec. (Context: `raw/19` Thrust 3 stateful-stars revision.)
  ...
  - [ ] **Spec B — library collection mode + SKILL wiring (NEXT).** Eager full-res download (rights-ungated, recorded), seed-import copy (`artist` + collection path → `images/user/`, originals untouched), execute the `DedupAction` file moves/deletes into `images/library/`, cross-run dedup, Phase-A SKILL wiring, e2e on the seeded `studies/cezanne/` (30 user images, real dup pairs). Final-review seams to handle: move-onto-delete ordering, manifest↔library-dir sync, metadata-merge authority.
