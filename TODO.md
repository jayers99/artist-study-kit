...
- [x] **Duplicate handling on re-query/ingest — BUILT (feature complete).** Same artwork arriving from different sources (or a re-imported user image) no longer re-adds a work / resets a starred one. Shipped as Spec A (engine) + Spec B (library collection mode), 314 tests, live Cezanne e2e passed. Remaining follow-on: QID-authoritative metadata override (below).
  ...
  - [ ] **QID-authoritative metadata override (deferred follow-on).** The dedup metadata merge is *existing-non-empty-wins*, so a seed image imported first with a filename-derived title keeps that title even after QID-rich discovery matches the work. Make an authoritative (QID-bearing) discovered value override a guessed/seed value. Needs its own small spec. (Context: Spec A `dedup.resolve` merge, Spec B final review.)
