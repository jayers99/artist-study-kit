...
6. **curation_interview** — gated on `selection.json`. Build the interview queue, then
   ...
   - `queue = build_queue(selected_rows(sel), work_meta)`. If a `study-briefs.json` already exists, `parse_briefs(json.load(...))` it and interview only `pending_targets(queue, briefs)` (resume).
